...
class ReEncryptModels extends Command
{
    ...
    protected function reEncryptModels(string $modelClass): void
    {
        $modelClass::unguarded(function () use ($modelClass) {
            $modelClass::query()
                ->when($this->option('with-trashed') && $modelClass::isSoftDeletable(), function (Builder $query) {
                    $query->withTrashed();
                })
                ->eachById(
    ...
    {
        return $models->every(fn (string $model): bool => !empty($model::currentEncrypter()->getPreviousKeys()));
    }
}
